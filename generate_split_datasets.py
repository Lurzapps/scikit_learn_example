...
# read the og file
with open(gl_vars.main_dataset_address, 'r') as og:
    # read all lines
...
if os.path.isfile(gl_vars.training_set_address):
    os.remove(gl_vars.training_set_address)
if os.path.isfile(gl_vars.testing_set_address):
    os.remove(gl_vars.testing_set_address)

# write these lists to the testing and training file
with open(gl_vars.training_set_address, 'a') as training:
    for i in range(len(training_list)):
        training.write(training_list[i])

with open(gl_vars.testing_set_address, 'a') as testing:
